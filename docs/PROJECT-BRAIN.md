...
- **ABV is a structural gap, not a scraping problem.** Almost no retailer or brand site publishes ABV consistently. It's legally required on Indian labels, so the fastest real path is **physical label photography/OCR at point of sale**, not more web research. **[RC3 status note, 2026-08-15]: this specific claim — that web research isn't a viable path for ABV — is now directly refuted for ABV. RC3.1–RC3.9's remote manufacturer-research methodology recovered real, cited ABV for 7 beers/29 SKUs from official brand sites, with zero physical fieldwork (which the founder separately ruled out entirely for this project). The finding was never really about ABV specifically — it was that the RC2.1-era "manufacturer evidence exhausted" assumption was too broad. The claim remains accurate for *calories* specifically: RC3 found real, repeated evidence that calorie data is genuinely harder to source remotely than ABV (wrong regulatory variants, incompatible units, or simple absence, across every brewery investigated). See §16's Fifth addendum for the full finding.**
...
**Fourth addendum, RC2 checkpoint (2026-08-14):** across the RC2.1–RC2.4 sessions, the Beer Knowledge Base grew from 65 to **278 beer entities**, grouping **915 of 1002 admitted SKUs (91.3%)** — up from 346 (34.5%) at the RC1 checkpoint. The **57-SKU/8-beer publishable figure is unchanged**, since RC2 was explicitly scoped to identity/grouping work, not ABV/calorie evidence gathering (RC2.1 established that manufacturer-tier remote evidence for ABV/calories is exhausted for nearly all remaining brands; that finding stands unchanged **[RC3 status note: this finding was directly refuted by RC3 — see the Fifth addendum below. It has not "stood unchanged"; it stood for roughly one RC cycle before being tested against real brewery data and failing.]**). RC2.4 produced a Final Exception Register cataloguing every one of the 51 remaining ordinary-backlog SKUs individually, each tagged with why it can't be grouped under the current evidence policy and whether manual (physical-label) observation would resolve it (yes, for all of them, but at low per-item value — 1–4 SKUs each — **[RC3 status note: "manual observation" here meant physical fieldwork, which the founder subsequently ruled out entirely; RC3 resolved the ABV portion of this same backlog by remote research instead, which this passage did not anticipate as a live option]**). Three categories of SKU are explicitly out of the Beer Knowledge Base's scope by founder decision, not backlog: Hill Station Hard Cider (16 SKUs), Grizly Hard Seltzer (17 SKUs), and Kingfisher Bohemia (3 SKUs, wine-varietal-named products) — all treated as upstream beer-classification issues. Two rows (Budweiser Magnum, Glenfiddich Experimental IPA) remain correctly unenriched as confirmed KSBCL-pipeline contamination. **The Beer Knowledge Base is considered complete under the current evidence policy** — remaining work is publication evidence gathering, not further identity research. **[RC3 status note: "under the current evidence policy" is the operative qualifier — RC3 changed what was believed possible under that same, unmodified policy, not the policy itself. See the Fifth addendum.]**

**Fifth addendum, RC3–RC6 checkpoint (2026-08-15):** this addendum does two things the discipline stated at the top of §15 requires when a later finding contradicts an earlier one: names the contradiction plainly, and records what actually shipped.

*Correcting the Fourth addendum's central assumption.* RC2.1's "manufacturer-tier remote evidence is exhausted for nearly all remaining brands" was never re-tested against real data before being carried forward as settled — it was carried forward as an assumption, restated as if it were still evidence. RC3.1 (Mount Everest Breweries) tested it directly and refuted it: real, citable, manufacturer-sourced ABV existed for six SKUs the assumption had already written off. RC3.2–RC3.9 confirmed this wasn't a one-off — the method is repeatable, subject to real per-brewery variance (a wrong regulatory-variant PDF at Molson Coors, a fully access-blocked domain at B9 Beverages/Bira 91, a resolved same-source internal contradiction at Mount Everest's own STOK page) — and produced, in total, cited ABV for **7 beers / 29 SKUs** previously marked `unknown`. This was accomplished entirely through web-based research; per the founder's own explicit, standing instruction issued mid-arc, physical fieldwork was never used and will never be part of this project.

*The evidence/publication decoupling this surfaced, and the decision it produced.* Recovering 29 SKUs' worth of ABV produced **zero** net publication gain, because `calories_per_100ml` — gated at the same blocking severity as ABV since Milestone 7 — remained unknown for all 7 beers and excluded them regardless. Direct inspection of `value_metrics.py`/`value_score.py` found zero references to calories anywhere in the app's core ranking computation. This evidence produced **Product Decisions Register D22** (full record: `PRODUCT-DECISIONS-REGISTER.md` Part 9, added this same checkpoint) — approved by the founder and implemented (`e2dd5d5`): calories moved from a publication-blocking rule to a warning, mirroring the stale-price mechanism already shipping. `Sku.calories` became nullable in both `models.py` and `lib/shared_domain/sku.dart`. The 7 evidenced beers were landed separately (`040904e`) from the gate-logic change itself, deliberately — evidence data and code logic are different units of work with different rollback boundaries.

*Current state, superseding the Fourth addendum's figures:* `catalog_version: 4`, **17 beers, 97 SKUs published** (up from 8/57), of which **40 published SKUs carry a `missing_calories` warning** rather than a known value. `docs/CATALOG-CONTRACT-1.0.md`, `docs/CATALOG-BUILDER-ARCHITECTURE.md`, and `docs/BEER-KNOWLEDGE-BASE-ARCHITECTURE.md` were synchronized to describe this shipped state in the same pass that produced this addendum (RC6.0) — before this pass, all three still described calories as mandatory/non-nullable, which was no longer true of the running code.

*What remains genuinely open, not resolved by any of the above:* `stok_wheat_fine`'s `style` field is still `unknown` — independent of D22, its 4 SKUs remain excluded on that separate, unrelated gate. The Product Decisions Register's own **D1** (Recommendation's ranking behavior for ABV-incomplete SKUs) is a different question from D22 and was not touched by it — see D1's own RC6.0 cross-reference note in the Register for why the two are easy to conflate and shouldn't be. No user-facing surface exists yet for a `null` calorie value — verified by exhaustive search of `lib/`, no screen reads `Sku.calories` at all today — so D22's consequences remain entirely latent pending any future feature that would display it.
