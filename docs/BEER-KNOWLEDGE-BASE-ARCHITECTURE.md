...
| `value_found` | **Required, except `access_blocked`** | string | The value that was found and rejected, exactly as found — not whatever value (if any) was ultimately curated instead. Optional specifically when `reason_type` is `access_blocked` (RC7.10/RC7.11): that reason type describes access failing before any value was ever read, so there is nothing to require — but a `value_found` that *is* given for an `access_blocked` entry is still validated as a non-empty string, same as every other reason type. |
...
**[RC7.10/RC7.11 addendum, 2026-08-17]:** the `value_found` question flagged just above is resolved. RC7.10 reviewed it against real repository evidence — the Bira 91/B9 Beverages case (`docs/PROJECT-BRAIN.md` §9/§16) is a genuinely different failure mode from the other five reason types: they all describe rejecting a value that was actually read, while `access_blocked` describes access failing before any value was ever read at all. RC7.11 implemented the fix: `value_found` is now optional specifically for `access_blocked` entries (see the field table above), enforced in `validate_rejected_evidence_entry` rather than loosened globally — every other reason type still requires a real, non-empty `value_found`, confirmed by regression tests covering all five. This does not, on its own, make the Bira 91 case recordable: its `subject_key` (which of 23 Bira91 beer files, or the brewery as a whole) and `observed_at` date remain unrecoverable from anything committed to this repository, and are not invented to close this out. The schema question and that specific data gap were always two different problems; only the first is resolved here.
