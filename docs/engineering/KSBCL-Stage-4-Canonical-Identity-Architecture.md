...
**Status: DRAFT — first pass, not reviewed, not frozen.** This document follows the same architecture standard applied to Stage 2 and Stage 3, but is explicitly not being proposed as ready to freeze. Every place a genuine judgment call was made rather than derived from an already-decided document is recorded in §13, "Architecture Questions Deferred to Freeze Review," and is not to be treated as settled by virtue of appearing in the body text above it. **Revised three times.** First, in §6/§7/§3.2, to correct a confirmed contradiction: the original draft removed master §4.4's price/`effective_date` review gate unconditionally, when only its cross-supplier configuration is actually addressed by the recorded Identity Decision — the same-supplier configuration master built that gate for was restored unchanged. Second, in §6/§7/§3.2/§8, to implement the recorded Stage 4 Identity Resolution Decision 2 (`KSBCL-Stage-4-Identity-Decision.md`): when a key matches more than one existing, live canonical product, Stage 4 does not choose among them — it defers to manual review. That behavior is a recorded product decision, not an architectural inference.

**Third revision — a genuine internal contradiction, found and corrected.** §6's matching mechanism defined its candidate pool as unrestricted across all prior runs (an "already-mapped" item_code explicitly includes one that's since been delisted), but §3.1's persisted schema never actually stored the matching-key fields needed to compare against such a candidate — meaning a real-world product reappearing under a new item_code, after its original item_code had been delisted, could never be recognized, with no review-queue flag either. §2.1, §3.1, §6, and §9 below now describe the corrected design: the four matching-key fields are persisted directly on `item_code_canonical_map.csv`, written once at first mapping and never recomputed. This is worth distinguishing explicitly from `KSBCL-Stage-4-Review-Closure-Report.md` §5's earlier, separate finding that "no storage was ever missing" for the matching key — that finding concerned multi-candidate ambiguity among candidates still live within the same run, where the key genuinely was derivable on demand; it does not cover a candidate that has already dropped out of a prior run's data entirely, which is what this revision addresses. See that report's own addendum for the disclosed distinction.
...
- **`structured_rows.csv`** (Stage 1, frozen, this `run_month`): joined on `item_code` for the fields Stage 3 does not carry — `supplier_name`, `supplier_code`, `declared_price`, `landed_cost`, `ksbcl_selling_price`, `mrp`, `effective_date`. Stage 4 is the first stage to need price and supplier information alongside normalized name/pack fields in one place. It is also the sole source for `item_status` (§3.1, §5): every already-mapped `item_code` is checked against this run's `structured_rows.csv` independently of whether it also appears in `normalized_rows.csv`, since the two files answer different questions — `structured_rows.csv` records whether KSBCL still lists the item at all, while `normalized_rows.csv` additionally depends on this run's Stage 2 classification outcome.
- **The prior run's `item_code_canonical_map.csv`** (Stage 4's own persistent state, `pricing_data/item_code_canonical_map.csv`, master §3) — every `ksbcl_item_code` ever resolved by a previous run, and what it was resolved to. On a first-ever run, this input is empty (bootstrap; §8). **This is now the sole source of an already-mapped item_code's matching key** (§3.1, §6) — an already-mapped candidate's key is read from this file's own persisted `normalized_name_key`/`pack_size_ml`/`pack_count`/`container_type` columns, never re-derived by joining against this run's `normalized_rows.csv`. A brand-new mapping still sources its key from this run's `normalized_rows.csv` at creation time, exactly as before; only an *already-mapped* candidate's key comes from the map itself.
...
| `normalized_name_key`, `pack_size_ml`, `pack_count`, `container_type` | The matching key (§4), copied verbatim from this item_code's `normalized_rows.csv` row **once, at first mapping, and never recomputed thereafter.** These are intentionally immutable — historical evidence supporting the identity decision recorded in `match_confidence`/`matched_rule` below, not a live cache of Stage 3's current output. No `normalization_rule_version` accompanies them: their correctness is defined as "what actually justified this decision when it was made," never "what today's normalization rules would produce." A future Stage 3 rule revision does not retroactively change them, by design — see §6 and §9. |
...
| `item_status` | `LIVE` \| `DELISTED` — at the item-code level only. Stage 4 sets this from whether the item_code is present in this run's `structured_rows.csv` (§2.1) — a genuinely-published-by-KSBCL signal, independent of whether that row also passed this run's Stage 2 classification into `normalized_rows.csv`. It does not compute or roll this up to a canonical-level status (that is Stage 5's job, master §4.5). |
...
- **Item-level status only.** Stage 4 sets `item_status` (`LIVE`/`DELISTED`) per mapped `item_code` based on that item_code's presence in the current run's `structured_rows.csv` (§2.1, §3.1) — not on its presence in `normalized_rows.csv`, which reflects this run's Stage 2 classification outcome rather than whether KSBCL still lists the item at all. It does not compute a canonical-product-level status — a canonical product with at least one `LIVE` item_code and several `DELISTED` ones is a fact Stage 5 rolls up (master §4.5), not one Stage 4 asserts.
...
1. **Already mapped** (present in the prior `item_code_canonical_map.csv` state, or mapped earlier in this same run) → no identity action; `last_seen_run_month` is updated, and `item_status` is set to `LIVE` or `DELISTED` per this item_code's presence in this run's `structured_rows.csv` (§2.1, §3.1) — checked independently of whether the item_code also appears in this run's `normalized_rows.csv`.
   - **An already-mapped item_code absent from this run's `normalized_rows.csv` entirely** (excluded by Stage 2 this run, or genuinely delisted) never enters the loop above, since that loop is driven by `normalized_rows.csv`'s row set (§2.1). It still requires `item_status` maintenance: checked against this run's `structured_rows.csv` exactly as above. `last_seen_run_month`, however, is **not** touched in this case — its meaning stays "last run this item_code was confirmed as beer via `normalized_rows.csv`," unchanged from before the `item_status` correction, since only `item_status`'s source was corrected, not this field's.
2. **Not yet mapped, and its matching key (§4) exactly equals the matching key of at least one already-mapped item_code.** An already-mapped item_code's key here is its *persisted* key (§3.1) — read directly from the map, never re-derived from this run's `normalized_rows.csv` — so an already-mapped item_code that has since been delisted (and is therefore absent from this run's data entirely) still participates as a match candidate. This is what makes item-code succession work across an intervening delisted month, not just within one run: a real product reappearing under a new item_code, after its original item_code dropped out of the data, is still recognized via the key recorded when it was first mapped. Two sub-cases, distinguished by whether that match is available from a supplier *other than* this item_code's own, or only from the *same* supplier:
   ...
   - **2b — only same-supplier matches exist.** If every already-mapped item_code sharing the exact key belongs to the *same* `supplier_code` as this item_code, master §4.4's original bar applies, unchanged: attach only if `mrp` *and* `effective_date` also match exactly against at least one of them (`match_confidence = deterministic_high`, `matched_rule = exact_key_match`). "Price" here is specifically `mrp`, not `declared_price`/`landed_cost`/`ksbcl_selling_price` — `mrp` is the one figure master treats as directly comparable across listings without a known case size (§1, §5.5, §4.6), and it is the field the worked example just below actually cites. If `mrp` or `effective_date` differs from all of them, this item_code does **not** attach — a **new** `canonical_product_id` is created instead, and a row is written to `canonical_resolution_review.csv` with `reason = possible_supersession` (§7) for manual review, exactly as master §4.4 specifies. **A same-supplier candidate that is itself delisted can never satisfy this gate** — its `mrp`/`effective_date` are not persisted anywhere on the map (only the matching-key fields are, §3.1), and none is fabricated here, consistent with never inventing a fact. Such a case therefore always falls to the `possible_supersession` branch: the reappearing item_code gets its own new canonical product, and a review row cites the delisted item_code, for a human to check against its own historical record. This is 2b's existing, already-frozen rule applied to a candidate it wasn't previously reachable for, not a new rule.
...
**Why 2a departs from master's original mechanism, and 2b does not.** Master §4.4's original design gated every automatic merge on a full match — key, price, *and* `effective_date` — flagging anything less for manual review. Under the original five-component key (supplier included), an exact key match could only ever occur between two listings from the *same* supplier, so that gate was, in substance, always a same-supplier test; master's own worked example for the "key matches, price/date differs" branch is a same-supplier pair (§1: "Budweiser Magnum Beer-CAN 330ML×24Cans" under item codes `2170900611` and `2171700211`, both supplier 0217, confirmed real: `mrp` ₹155.00/2025-08-25 and ₹140.00/2026-05-12 — their `declared_price` values, ₹1,050.18/₹1,051.00, are close but not equal, and are not the figures being cited here). Removing `supplier_code` from the key, per the recorded Identity Decision, makes an exact key match reachable across different suppliers for the first time — and only for that new configuration does the Identity Decision say anything at all. It says nothing about the same-supplier configuration; master's original text, including its explicit "never automatic" language for a same-supplier key match with a price or date difference, governs that case unchanged, which is what 2b restores.

**Correction (Stage 4 implementation, 2026-06 real run):** this pair is *not*, in fact, held apart in the current real dataset — a third supplier, S P R Distilleries (`supplier_code 0212`, item_code `2120902011`), independently lists the identical product under the same matching key. Per the precedence rule stated two paragraphs below ("2a's single-candidate branch governs" whenever both a qualifying cross-supplier match and a same-supplier mismatch exist for the same key), all three item_codes correctly resolve to one canonical product via 2a, not 2b — the cross-supplier evidence from the third listing is sufficient on its own under the Identity Decision, regardless of the same-supplier pair's own `mrp`/`effective_date` disagreement. The 2b mechanism itself is correctly implemented and was directly exercised, and confirmed working as specified, on other same-supplier-only pairs in the same real run; this specific pair was simply never a 2b case to begin with. The paragraph above is left in place because the same-supplier `mrp`/`effective_date` values it cites are real and correctly illustrate what 2b compares — only the "held apart" conclusion, which depended on assuming no third listing existed, was wrong.
...
**Re-running an already-processed month** against the same prior map state deterministically reproduces the same `item_code_canonical_map.csv` and `canonical_resolution_review.csv` for that run — never appends, never duplicates, mirroring Stage 1's and Stage 3's own idempotency bar. Mechanically, this requires comparing against the map state as it stood *before* this run_month was ever processed, not whatever is currently on disk — which, on a genuine rerun, already includes this run_month's own effects from an earlier attempt. `first_seen_run_month` is exactly the signal that distinguishes the two: any entry whose `first_seen_run_month` equals this run's `run_month` was created by this run_month's own processing (whether on the first attempt or a retry) and is treated as not-yet-mapped again — recomputed fresh, in the same deterministic order, against the same true-prior-state candidates — rather than as "already mapped." This is what makes a rerun reproduce the same `canonical_resolution_review.csv` rather than silently losing that run's review rows once its own item_codes are already mapped.

---

## 9. Auditability

**What is directly auditable from `item_code_canonical_map.csv` as specified:** which run first saw a given item_code (`first_seen_run_month`); whether an item_code's canonical product was assigned by exact-key mechanism or by a human (`match_confidence`, `matched_rule`); the item_code's own supplier and current liveness; and now, the exact matching key that justified that decision (`normalized_name_key`, `pack_size_ml`, `pack_count`, `container_type`), directly on the row, without needing to re-read `normalized_rows.csv` from whichever run first mapped it. This closes what was previously a real audit gap: an already-mapped item_code's key was only ever visible for as long as it kept appearing in a current run's `normalized_rows.csv` — a delisted item_code's key was not reconstructable from the map at all. Combined with the run-dated archive already required by master §3, the full sequence of monthly map states is reconstructable without re-reading the source PDF, the same reconstructability guarantee every earlier stage's audit artifact provides.
