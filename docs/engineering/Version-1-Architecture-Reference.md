...
**Comparison** — blocked, canon, primarily: the Comparison Screen Contract's own Section 11 leaves 3+-candidate scaling unresolved, and `RecommendationTie` can produce any cardinality, so even the one entry point technically reachable today (`Recommendation → Comparison` from a genuine tie) can't guarantee the two-candidate case every worked example in the canon assumes. Also blocked, repository, as an additional limitation: it depends on Trade-off/Tie-handling's trade-off half, which is unreachable for the same reason above; and even setting the scaling gap aside, that one reachable entry point has nothing further to add, since a tie's candidates are by construction equal on every measured axis — building it would duplicate Recommendation's own tie rendering, violating Information Architecture's anti-duplication rule.
