...
## Addendum — a distinct scenario found after this review closed

This report is a historical record of the review as it happened and is not being rewritten. Recorded here instead: a later, independent review found a genuine architectural contradiction this review did not surface — Stage 4's §6 defined its matching-candidate pool as unrestricted across all prior runs, including a delisted item_code, while §3.1's persisted schema never actually stored the matching-key fields needed to compare against one. The consequence: a real-world product reappearing under a new item_code, after its original item_code had already been delisted, could not be recognized, with no review-queue flag either.

This is a different scenario from §5's rejected "key registry" finding above, worth distinguishing explicitly rather than leaving the two sitting in unacknowledged tension: that finding concerned multi-candidate ambiguity among candidates still live within the *same* run, where the key was genuinely derivable on demand from that run's own `normalized_rows.csv` — correctly rejected on those terms. It never addressed a candidate that has already dropped out of a *prior* run's data entirely, where no such on-demand derivation is possible. Neither this review's own adversarial passes nor its freeze-review question list (§8 above) named this specific case.

The correction — persisting the four matching-key fields directly on `item_code_canonical_map.csv`, as immutable historical evidence rather than a live cache — is recorded in `KSBCL-Stage-4-Canonical-Identity-Architecture.md`'s own third revision note.
