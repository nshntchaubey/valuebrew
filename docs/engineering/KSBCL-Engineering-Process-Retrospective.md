...
## 10. Addendum — five lessons from correcting an already-frozen stage

Everything above was extracted from how Stages 2–4 got *built*. This addendum is extracted from a different kind of event this workflow hadn't yet produced a lesson for: correcting a defect in a stage already frozen, reviewed, and implemented. Each lesson below survived being checked against the incident that produced it and generalized past that incident's specifics — none of them are instructions for what to do about that specific defect, which is closed.

**1. A mechanism's stated scope must be checked against its own schema's literal capacity — not assumed consistent because both were separately reviewed.** What happened: an architecture section described a mechanism operating over "every prior candidate," while the same document's own schema section, defining what that prior state actually persists, didn't carry what the mechanism needed once time had passed. Why: the mechanism and the schema were drafted and reviewed as two separate, individually-reasonable pieces of the same document, and nobody cross-checked one against the other's literal words. Permanent practice: whenever a mechanism describes what it operates over, trace that claim against the literal, enumerated schema of the state it depends on, as an explicit step — not an assumption that internal consistency follows from each section being individually correct.

**2. A closed finding's conclusion transfers only as far as the precondition that made it true.** What happened: a review correctly rejected a proposed fix for one scenario because the fact in question was "derivable on demand" — true for that scenario specifically, because the data was still observable. A different, similar-sounding scenario existed where that same precondition didn't hold, and the closed finding's conclusion could have been silently assumed to cover it too. Why: two scenarios shared surface resemblance ("is X missing from storage?") while differing in the one fact that actually determined the answer. Permanent practice: before citing a previously-closed finding as still applicable to a new situation, restate the specific precondition that made the original finding true, and check it holds in the new situation — never reuse a conclusion by resemblance alone.

**3. Field mutability has no syntactic marker — it must be stated explicitly wherever a persisted field is introduced.** What happened: a newly persisted field needed a decision about whether it behaves like current-state (recomputed every run) or like a recorded decision (frozen until deliberate action) — a distinction invisible in the code itself, since both are "just a field on a row." Why: without an explicit statement, both the person writing the field and anyone reviewing it have to infer intent, and can infer differently. Permanent practice: every new persisted field is labeled, in the document that introduces it, as exactly one of "current-state, recomputed every run" or "decided-once, frozen until deliberate action" — never left to be inferred from the surrounding code.

**4. Confirming several new safety mechanisms each exist is not the same as confirming they compose correctly.** What happened: a review required two new safety steps be added to an operation. Both were added. Neither was individually wrong. Run in sequence, twice, they interacted in a way that defeated one of them. Why: review verified presence of each requirement, not the behavior of the combined operation across repeated execution — presence checks don't compose. Permanent practice: whenever a review adds more than one new mechanism to the same operation, the review explicitly traces the composed behavior across at least two invocations before considering either requirement closed — not just confirms each one was individually incorporated.

**5. Independent review must be a deliberate decision made per category of artifact, not an inherited default.** What happened: this project's own independent-review discipline — a fresh, uninvolved reviewer checking a claim before it's trusted — was applied rigorously to architecture and to application code, but a one-time, hard-to-undo migration script defaulted to being checked only by whoever wrote it, until a later, separately-scoped audit happened to catch what that missed. Why: independent review had never been explicitly extended to this category of artifact, so the category quietly inherited "self-verification is fine" rather than anyone deciding that on purpose. Permanent practice: any new category of artifact this project produces gets an explicit decision about whether it requires independent review — never a silent default.

### Where these lessons belong

None of the five are promoted into `KSBCL-Repository-Governance.md` here. That document's own standard, demonstrated throughout its own eleven conventions, is extraction from *repeated* practice — every existing convention cites at least two or three independent instances, never one. A single incident, however instructive, doesn't yet meet that bar. These five stay here, in the process record, until (and unless) a future stage independently reproduces the same shape of problem — at which point the recurrence, not this write-up, is what would justify promoting the corresponding lesson into Repository Governance as a twelfth convention, via the same extraction discipline that document already documents about itself.
