...
`--observed-at` defaults to today; add `--recheck-after YYYY-MM-DD` when the rejection reason (`access_blocked`, most often) implies it's worth looking again later. `--value-found` is required for every `--reason-type` except `access_blocked` — that one describes access failing before you ever read a value at all, so omit `--value-found` entirely for it rather than writing a placeholder (**[RC7.11]**). The command refuses to write anything that fails validation — a `--subject-key` that doesn't resolve to a real `beer_key`/`brewery`, an unrecognized `--reason-type`, a missing `--value-found` for any reason type that needs one, or a duplicate of an entry already recorded (same subject, field, source, value, and reason) all fail loudly, before anything is written, the same discipline `create_beer.py`/`update_beer.py` already use.
...
**This file was empty of historical entries at RC7.7, on purpose.** RC7.7 built the recording infrastructure only — it did not, and was explicitly instructed not to, invent or reconstruct the rejected-evidence cases from before it existed. If you're the one who remembers a real historical rejection from before this tooling existed, don't hand-write it into the YAML — that's exactly the "never hand-edit" rule above. Record it through the command like any other entry.

**[RC7.8 addendum, 2026-08-17]:** two real historical cases (`miller_high_life_premium`, `stok_strong_fine`) were recovered by re-reading already-accepted `enrichment/beers/*.yaml` citations that happened to narrate a rejected alternate value inline, and recorded through the command above. RC7.6's originally-referenced six-case list itself was never located and remains outstanding — see `docs/BEER-KNOWLEDGE-BASE-ARCHITECTURE.md` Part 11's own RC7.8 addendum for the full audit and what's still missing.
